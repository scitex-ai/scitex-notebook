...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-notebook's —
...
# so only scitex-notebook's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_notebook-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# scitex-notebook's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-notebook + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-notebook's historical bare-uv/pip workflow so a
...
# scitex-notebook's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-notebook style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-notebook-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-notebook's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_notebook --cov-report=xml --cov-report=term \
    -p no:cacheprovider
