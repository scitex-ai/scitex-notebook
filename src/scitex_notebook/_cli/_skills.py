"""`scitex-notebook skills` — list / get / install agent-facing skills.

Self-contained. No scitex-dev runtime dep — walks the package's own
`_skills/scitex-notebook/` directory directly.
"""
...
def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-notebook/` directory."""
    import scitex_notebook

    pkg_dir = Path(scitex_notebook.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-notebook/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with scitex-notebook.

    \b
    Examples:
      $ scitex-notebook skills list
      $ scitex-notebook skills get 01_installation
      $ scitex-notebook skills install                  # → ~/.scitex/dev/skills/scitex-notebook/
      $ scitex-notebook skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_list(as_json: bool) -> None:
    ...
    Example:
      $ scitex-notebook skills list
      $ scitex-notebook skills list --json
    """
    root = _skills_root()
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-notebook skills get 01_installation
      $ scitex-notebook skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-notebook/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/scitex-notebook/` dir to
    ~/.scitex/dev/skills/scitex-notebook/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ scitex-notebook skills install
      $ scitex-notebook skills install --claude-symlink
      $ scitex-notebook skills install --no-link --dest /tmp/scitex-notebook-skills
    """
